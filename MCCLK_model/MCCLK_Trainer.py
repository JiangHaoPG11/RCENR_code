...
class Trainer():
    def __init__(self, args, MCCLK_model, optimizer_MMCCLK, data):
        self.args = args
        self.MCCLK_model = MCCLK_model
        self.optimizer_MMCCLK = optimizer_MMCCLK
    ...
    def optimize_MMCCLK(self, loss,):
        self.optimizer_MMCCLK.zero_grad()
        loss.backward(retain_graph=True)
        self.optimizer_MMCCLK.step()

    def _train_epoch(self):
        self.MCCLK_model.train()
        rec_all_loss_list = []
        auc_list = []
        pbar = tqdm(total=self.traindata_size)
        for data in self.train_dataloader:
            candidate_newindex, user_index, user_clicked_newindex, label = data
            loss, scores, rec_loss, emb_loss = self.MCCLK_model(user_index, candidate_newindex, label)
            print(rec_loss)
            print(emb_loss)
    ...
    def _save_checkpoint(self, epoch):
        state_agent = self.MCCLK_model.state_dict()
    ...
    def test(self):
        ...
        with no_grad():
            for data in self.test_dataloader:
                ...
                scores = self.MCCLK_model.test(user_index, candidate_newindex)
