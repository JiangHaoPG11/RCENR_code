...
class Trainer():
    def __init__(self, args, KGAT_VAE_model, optimizer_KGAT_VAE, data):
        self.args = args
        self.KGAT_VAE_model = KGAT_VAE_model
        self.optimizer_KGAT_VAE = optimizer_KGAT_VAE
    ...
    def optimize_KGAT_VAE(self, loss,):
        self.optimizer_KGAT_VAE.zero_grad()
        loss.backward()
        self.optimizer_KGAT_VAE.step()

    def _train_epoch(self):
        self.KGAT_VAE_model.train()
        rec_all_loss_list = []
        auc_list = []
        pbar = tqdm(total=self.traindata_size)
        for data in self.train_dataloader:
            candidate_newindex, user_index, user_clicked_newindex, label = data
            loss, scores, rec_loss, emb_loss = self.KGAT_VAE_model(user_index, candidate_newindex, user_clicked_newindex, label)
            self.optimize_KGAT_VAE(loss)
    ...
    def _save_checkpoint(self, epoch):
        state_agent = self.KGAT_VAE_model.state_dict()
    ...
    def test(self):
        ...
        with no_grad():
            for data in self.test_dataloader:
                ...
                scores = self.KGAT_VAE_model.test(user_index, candidate_newindex, user_clicked_newindex)
